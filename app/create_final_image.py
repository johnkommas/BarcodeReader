...
def run(df, file_name, price):
    ...
    my_image.paste(overlay, (2270, 388), mask=overlay)

    # TAGS
    overlay = Image.open(f"{path}/images/new_product.png").convert("RGBA")
    overlay = Image.open(f"{path}/images/special_offer.png").convert("RGBA")
    overlay = Image.open(f"{path}/images/vegan.png").convert("RGBA")
    overlay = Image.open(f"{path}/images/Bio.png").convert("RGBA")
    overlay = Image.open(f"{path}/images/100_bio.png").convert("RGBA")
    overlay = Image.open(f"{path}/images/gluten_free.png").convert("RGBA")
    size = (overlay.size[0] // 1, overlay.size[1] // 1)
    overlay = overlay.resize(size, Image.ANTIALIAS)
    if len(str(euro_price)) == 1:
        image_editable.text((1346 - 30, 80),  str(euro_price) + ".", (244, 36, 7), font=euro_font)
        my_image.paste(overlay, (186 - 30, 211), mask=overlay)
    elif len(str(euro_price)) == 2:
        image_editable.text((930 - 30, 80),  str(euro_price) + ".", (244, 36, 7), font=euro_font)
        my_image.paste(overlay, (68 - 30, 211), mask=overlay)
    elif len(str(euro_price)) == 3:
        image_editable.text((555 - 30, 80),  str(euro_price) + ".", (244, 36, 7), font=euro_font)
        my_image.paste(overlay, (-212, 211), mask=overlay)
    # Εμφανίζει ένα δεκαδικό όταν η τιμή είναι π.χ. 1.0 έτσι προσθέτουμε ακόμα ένα 0
    image_editable.text((1755 - 30, 195), (str(copper_price) if len(str(copper_price)) == 2 else str(copper_price) + "0"), (244, 36, 7), font=copper_font)
    image_editable.text((2087 - 30, 340), "€", (244, 36, 7), font=euro_sign_font)
    image_editable.text((2141 -30 , 261), str(fMUCode).lower(), (0, 0, 0), font=fMUCode_font)
